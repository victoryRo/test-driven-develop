...
// Add is the method that processes the addition operation
func (e *Engine) Add(x, y float64) (*float64, error) {
	result := y + x
	return &result, nil
}

// Sub is the method that processes the subtract operation
func (e *Engine) Sub(x, y float64) (*float64, error) {
	result := x - y
	return &result, nil
}

// Mult is the method that proccess multiply operation
func (e *Engine) Mult(x, y float64) (*float64, error) {
	result := x * y
	return &result, nil
}

// Div is the method that proccess divide operation
