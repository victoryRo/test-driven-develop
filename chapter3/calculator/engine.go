...
type Engine struct {
	expectedLength int
	validOperation map[string]func(x, y float64) float64
}
...
func (e *Engine) ProcessOperation(operation Operation) (*string, error) {
	f, ok := e.validOperation[operation.Operator]
	if !ok {
